...
class ModelbeneficiaryproductDownload extends Model
{
    public function addDownload($data)
    {
        $this->event->trigger('pre.customer.add.download', $data);
        ...
        $this->event->trigger('post.customer.add.download', $download_id);
    ...
    {
        $this->event->trigger('pre.customer.edit.download', $data);
        ...
        $this->event->trigger('post.customer.edit.download', $download_id);
    }

    public function deleteDownload($download_id)
    {
        $this->event->trigger('pre.customer.delete.download', $download_id);

        $this->db->query('DELETE FROM '.DB_PREFIX."download WHERE download_id = '".(int) $download_id."'");
        $this->db->query('DELETE FROM '.DB_PREFIX."download_description WHERE download_id = '".(int) $download_id."'");

        $this->event->trigger('post.customer.delete.download', $download_id);
    }
