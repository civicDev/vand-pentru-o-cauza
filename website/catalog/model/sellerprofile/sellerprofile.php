...
class Modelsellerprofilesellerprofile extends Model
{
    ...
    {
        $sql = 'SELECT * FROM '.DB_PREFIX.'seller_group cg
        LEFT JOIN '.DB_PREFIX.'seller_group_description cgd
        ON (cg.seller_group_id = cgd.seller_group_id)
        LEFT JOIN (
        SELECT wcd.commission_rate_id,wc.seller_group_id,wcd.rate,wcd.type,wcd.name as commission_name  FROM '.DB_PREFIX.'commission_rate_to_seller_group wc
  			LEFT JOIN '.DB_PREFIX."commission_rate wcd ON (wc.commission_rate_id = wcd.commission_rate_id)
        ) cr ON (cg.seller_group_id = cr.seller_group_id)
