...
class ControllersellerproductProduct extends Controller
{
    ...
    protected function getList()
    {
        ...
        if (file_exists(DIR_TEMPLATE.$this->config->get('config_template').'/template/sellerproduct/product_list.tpl')) {
            $this->response->setOutput($this->load->view($this->config->get('config_template').'/template/sellerproduct/product_list.tpl', $data));
        } else {
            $this->response->setOutput($this->load->view('default/template/sellerproduct/product_list.tpl', $data));
        }
    }
    ...
    protected function getForm()
    {
        ...
        if (!$isproductseller) {
            ...
            $data['config_multiseller_category'] = $this->config->get('config_multiseller_category');
        ...
        $this->load->model('sellerproduct/download');
            ...
            if (file_exists(DIR_TEMPLATE.$this->config->get('config_template').'/template/sellerproduct/product_form.tpl')) {
                $this->response->setOutput($this->load->view($this->config->get('config_template').'/template/sellerproduct/product_form.tpl', $data));
            } else {
                $this->response->setOutput($this->load->view('default/template/sellerproduct/product_form.tpl', $data));
            }
        }
    }
    ...
    public function category_autocomplete()
    {
        ...
        if (isset($this->request->get['filter_name'])) {
            ...
            $sellerCategories = $this->model_sellerproduct_product->getsellerCategories($this->customer->getId());

            if (!empty($sellerCategories)) {
                $results = $this->model_sellerproduct_category->getSellerCategories($filter_data);
            } else {
                $sellergroupCategories = $this->model_sellerproduct_product->getsellergroupCategories($this->customer->getSellerGroupId());

                if (!empty($sellergroupCategories)) {
