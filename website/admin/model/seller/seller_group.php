...
class ModelSellersellerGroup extends Model
{
    ...
    {
        $this->db->query('INSERT INTO '.DB_PREFIX."seller_group SET  sort_order = '".(int) $data['sort_order']."',product_limit = '".(int) $data['product_limit']."' ,subscription_duration = '".(int) $data['subscription_duration']."' ,subscription_price = '".(int) $data['subscription_price']."', status = '".(int) $data['status']."'");
        ...
        if (isset($data['seller_commission'])) {
            foreach ($data['seller_commission'] as $seller_commission_id) {
                $this->db->query('INSERT INTO '.DB_PREFIX."commission_rate_to_seller_group SET seller_group_id = '".(int) $seller_group_id."', commission_rate_id = '".(int) $seller_commission_id."'");
            }
        }
    ...
    {
        $this->db->query('UPDATE '.DB_PREFIX."seller_group SET sort_order = '".(int) $data['sort_order']."',product_limit = '".(int) $data['product_limit']."' ,subscription_duration = '".(int) $data['subscription_duration']."' ,subscription_price = '".(int) $data['subscription_price']."', status = '".(int) $data['status']."'WHERE seller_group_id = '".(int) $seller_group_id."'");
        ...
        $this->db->query('DELETE FROM '.DB_PREFIX."commission_rate_to_seller_group WHERE seller_group_id = '".(int) $seller_group_id."'");
    ...
    public function getcommissionsellergroup($seller_group_id)
    {
        $seller_data = array();

        $query = $this->db->query('SELECT * FROM '.DB_PREFIX."commission_rate_to_seller_group WHERE seller_group_id = '".(int) $seller_group_id."'");

        foreach ($query->rows as $result) {
            $seller_data[] = $result['commission_rate_id'];
        }
