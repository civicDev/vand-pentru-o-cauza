...
class ModelSellerOrder extends Model
{
    ...
    {
        $sql = "
		SELECT *, SUM(op.total) as totals,o.order_id as order_id,o.date_added, CONCAT(c.firstname, ' ', c.lastname) AS seller,c.customer_id as customer_id ,
		...
		LEFT JOIN  `'.DB_PREFIX.'order_seller_settlement` oss ON (o.order_id = oss.order_id)
		';
    ...
    {
        $sql = '
		SELECT oss.settlement FROM '.DB_PREFIX.'order_sellerhistory  osh
		LEFT JOIN  `'.DB_PREFIX."order_seller_settlement` oss ON (osh.order_id = oss.order_id)
    ...
    public function addOrderSellerHistory($seller_id, $order_id, $settlement = true, $order_status_id, $comment = '', $notify = false)
    {
        $this->db->query('INSERT INTO '.DB_PREFIX."order_sellerhistory SET order_id = '".(int) $order_id."', order_status_id = '".(int) $order_status_id."', notify = '".(int) $notify."', seller_id = '".(int) $seller_id."',settlement = '".(int) $settlement."',comment = '".$this->db->escape($comment)."', date_added = NOW()");
    }
}
