...
class ModelbeneficiarydashboardCustomer extends Model
{
    public function getTotalCustomersByDay()
    {
		...
		WHERE DATE(c.date_added) = DATE(NOW()) GROUP BY HOUR(c.date_added) ORDER BY c.date_added ASC');
    ...
    public function getTotalCustomersByWeek()
    {
		...
		FROM `'.DB_PREFIX.'customer` c
		LEFT JOIN '.DB_PREFIX."beneficiary_group_description cgd ON (c.beneficiary_group_id = cgd.beneficiary_group_id)
		WHERE DATE(c.date_added) >= DATE('".$this->db->escape(date('Y-m-d', $date_start))."') GROUP BY DAYNAME(c.date_added)");
    ...
    public function getTotalCustomersByMonth()
    {
		...
		FROM `'.DB_PREFIX.'customer` c
		LEFT JOIN '.DB_PREFIX."beneficiary_group_description cgd ON (c.beneficiary_group_id = cgd.beneficiary_group_id)
		WHERE DATE(c.date_added) >= '".$this->db->escape(date('Y').'-'.date('m').'-1')."' GROUP BY DATE(c.date_added)");
    ...
    public function getTotalCustomersByYear()
    {
        ...
        $query = $this->db->query('SELECT COUNT(*) AS total, 
		date_added FROM `'.DB_PREFIX.'customer` c
		LEFT JOIN '.DB_PREFIX.'beneficiary_group_description cgd ON (c.beneficiary_group_id = cgd.beneficiary_group_id)
		WHERE YEAR(c.date_added) = YEAR(NOW()) GROUP BY MONTH(c.date_added)');
    ...
    public function getOrders($data = array())
    {
		...
		CONCAT(c.firstname, ' ', c.lastname) AS customer, c.email, cgd.name AS customer_group, c.status, 
		COUNT(o.order_id) AS orders, SUM(op.quantity) AS products, SUM(o.total) AS `total` 
		FROM `".DB_PREFIX.'customer` c 
    ...
    {
        $sql = 'SELECT COUNT(DISTINCT o.customer_id) AS total FROM `'.DB_PREFIX.'product_to_beneficiary` cp
		LEFT JOIN  `'.DB_PREFIX.'order_product` op ON (cp.product_id = op.product_id) 
		LEFT JOIN  `'.DB_PREFIX."order` o ON (op.order_id = o.order_id) 
    ...
    {
        $sql = "SELECT cr.customer_id, 
		CONCAT(c.firstname, ' ', c.lastname) AS customer, c.email, cgd.name AS customer_group, c.status, 
		SUM(cr.points) AS points, COUNT(o.order_id) AS orders, SUM(o.total) AS total 
		FROM ".DB_PREFIX.'customer_reward cr 
		LEFT JOIN `'.DB_PREFIX.'customer` c ON (cr.customer_id = c.customer_id) 
		LEFT JOIN '.DB_PREFIX.'beneficiary_group_description cgd ON (c.beneficiary_group_id = cgd.beneficiary_group_id) 
		LEFT JOIN `'.DB_PREFIX."order` o ON (cr.order_id = o.order_id) WHERE cgd.language_id = '".(int) $this->config->get('config_language_id')."'";
    ...
    {
        $sql = 'SELECT COUNT(DISTINCT cr.customer_id) AS total FROM `'.DB_PREFIX.'customer_reward` cr
		LEFT JOIN `'.DB_PREFIX.'customer` c ON (cr.customer_id = c.customer_id) 
		LEFT JOIN '.DB_PREFIX.'beneficiary_group_description cgd ON (c.beneficiary_group_id = cgd.beneficiary_group_id)
		';
    ...
    public function getCredit($data = array())
    {
		...
		CONCAT(c.firstname, ' ', c.lastname) AS customer, c.email, cgd.name AS customer_group, c.status, SUM(ct.amount) AS total 
		FROM `".DB_PREFIX.'customer_transaction` ct 
    ...
    {
        $sql = 'SELECT COUNT(DISTINCT ct.customer_id) AS total FROM `'.DB_PREFIX.'customer_transaction` ct
		LEFT JOIN `'.DB_PREFIX.'customer` c ON (ct.customer_id = c.customer_id) 
		LEFT JOIN '.DB_PREFIX.'beneficiary_group_description cgd ON (c.beneficiary_group_id = cgd.beneficiary_group_id)
		';
    ...
    {
        $sql = '
		SELECT co.ip, co.customer_id, co.url, co.referer, co.date_added 
		FROM '.DB_PREFIX.'customer_online co 
		
		LEFT JOIN '.DB_PREFIX.'customer c ON (co.customer_id = c.customer_id)
		LEFT JOIN '.DB_PREFIX."beneficiary_group_description cgd ON (c.beneficiary_group_id = cgd.beneficiary_group_id)
		WHERE co.customer_id != '0' AND cgd.beneficiary_group_id 
    ...
    {
        $sql = 'SELECT COUNT(*) AS total FROM `'.DB_PREFIX.'customer_online` co 
		LEFT JOIN '.DB_PREFIX.'customer c ON (co.customer_id = c.customer_id)
		LEFT JOIN '.DB_PREFIX."beneficiary_group_description cgd ON (c.beneficiary_group_id = cgd.beneficiary_group_id)
		WHERE co.customer_id != '0' AND cgd.beneficiary_group_id != '0'
		";
    ...
    {
        $sql = 'SELECT ca.activity_id, ca.customer_id, ca.key, ca.data, ca.ip, ca.date_added 
FROM '.DB_PREFIX.'customer c
LEFT JOIN '.DB_PREFIX.'beneficiary_group_description cgd ON (c.beneficiary_group_id = cgd.beneficiary_group_id)
LEFT JOIN '.DB_PREFIX.'customer_activity ca  ON (ca.customer_id = c.customer_id)
		';
