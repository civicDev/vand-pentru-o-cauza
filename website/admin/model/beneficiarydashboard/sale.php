...
class ModelbeneficiarydashboardSale extends Model
{
    public function getTotalSales($data = array())
    {
        $sql = 'SELECT SUM(total) AS total FROM `'.DB_PREFIX."order` o WHERE order_status_id > '0'";
    ...
    {
        $query = $this->db->query('SELECT COUNT(*) AS total, SUM(o.total) AS amount, c.iso_code_2 FROM `'.DB_PREFIX.'product_to_beneficiary` cp
		LEFT JOIN	`'.DB_PREFIX.'order_product` op ON (cp.product_id = op.product_id)
		LEFT JOIN  `'.DB_PREFIX.'order` o  ON (o.order_id = op.order_id) LEFT JOIN `'.DB_PREFIX."country` c ON (o.payment_country_id = c.country_id) WHERE o.order_status_id > '0' GROUP BY o.payment_country_id");
    ...
    public function getOrders($data = array())
    {
		...
		WHERE ot.order_id = o.order_id AND ot.code = 'tax' GROUP BY ot.order_id)) AS tax, SUM(o.total) AS `total`
		FROM `".DB_PREFIX.'product_to_beneficiary` cp
		LEFT JOIN	`'.DB_PREFIX.'order_product` op ON (cp.product_id = op.product_id)
    ...
    public function getTotalOrders($data = array())
    {
        ...
        switch ($group) {
            case 'day';
                $sql = 'SELECT COUNT(DISTINCT YEAR(date_added), MONTH(date_added), DAY(date_added)) AS total FROM `'.DB_PREFIX.'product_to_beneficiary` cp
		LEFT JOIN	`'.DB_PREFIX.'order_product` op ON (cp.product_id = op.product_id)
		LEFT JOIN  `'.DB_PREFIX.'order` o  ON (o.order_id = op.order_id) ';
                break;
            default:
            case 'week':
                $sql = 'SELECT COUNT(DISTINCT YEAR(date_added), WEEK(date_added)) AS total FROM `'.DB_PREFIX.'product_to_beneficiary` cp
		LEFT JOIN	`'.DB_PREFIX.'order_product` op ON (cp.product_id = op.product_id)
		LEFT JOIN  `'.DB_PREFIX.'order` o  ON (o.order_id = op.order_id) ';
                break;
            case 'month':
                $sql = 'SELECT COUNT(DISTINCT YEAR(date_added), MONTH(date_added)) AS total FROM `'.DB_PREFIX.'product_to_beneficiary` cp
		LEFT JOIN	`'.DB_PREFIX.'order_product` op ON (cp.product_id = op.product_id)
		LEFT JOIN  `'.DB_PREFIX.'order` o  ON (o.order_id = op.order_id) ';
                break;
            case 'year':
                $sql = 'SELECT COUNT(DISTINCT YEAR(date_added)) AS total FROM `'.DB_PREFIX.'product_to_beneficiary` cp
		LEFT JOIN	`'.DB_PREFIX.'order_product` op ON (cp.product_id = op.product_id)
		LEFT JOIN  `'.DB_PREFIX.'order` o  ON (o.order_id = op.order_id) ';
                break;
        }
    ...
    {
        $sql = 'SELECT MIN(o.date_added) AS date_start, MAX(o.date_added) AS date_end, ot.title, SUM(ot.value) AS total, COUNT(o.order_id) AS `orders` FROM `'.DB_PREFIX.'product_to_beneficiary` cp
		LEFT JOIN	`'.DB_PREFIX.'order_product` op ON (cp.product_id = op.product_id)
		LEFT JOIN  `'.DB_PREFIX.'order` o  ON (o.order_id = op.order_id) LEFT JOIN `'.DB_PREFIX."order_total` ot ON (ot.order_id = o.order_id) WHERE ot.code = 'tax'";
    ...
    public function getTotalTaxes($data = array())
    {
        ...
        switch ($group) {
            case 'day';
                $sql = 'SELECT COUNT(DISTINCT YEAR(o.date_added), MONTH(o.date_added), DAY(o.date_added), ot.title) AS total FROM `'.DB_PREFIX.'product_to_beneficiary` cp
		LEFT JOIN	`'.DB_PREFIX.'order_product` op ON (cp.product_id = op.product_id)
		LEFT JOIN  `'.DB_PREFIX.'order` o  ON (o.order_id = op.order_id)';
                break;
            default:
            case 'week':
                $sql = 'SELECT COUNT(DISTINCT YEAR(o.date_added), WEEK(o.date_added), ot.title) AS total FROM `'.DB_PREFIX.'product_to_beneficiary` cp
		LEFT JOIN	`'.DB_PREFIX.'order_product` op ON (cp.product_id = op.product_id)
		LEFT JOIN  `'.DB_PREFIX.'order` o  ON (o.order_id = op.order_id)';
                break;
            case 'month':
                $sql = 'SELECT COUNT(DISTINCT YEAR(o.date_added), MONTH(o.date_added), ot.title) AS total FROM `'.DB_PREFIX.'product_to_beneficiary` cp
		LEFT JOIN	`'.DB_PREFIX.'order_product` op ON (cp.product_id = op.product_id)
		LEFT JOIN  `'.DB_PREFIX.'order` o  ON (o.order_id = op.order_id)';
                break;
            case 'year':
                $sql = 'SELECT COUNT(DISTINCT YEAR(o.date_added), ot.title) AS total FROM `'.DB_PREFIX.'product_to_beneficiary` cp
		LEFT JOIN	`'.DB_PREFIX.'order_product` op ON (cp.product_id = op.product_id)
		LEFT JOIN  `'.DB_PREFIX.'order` o  ON (o.order_id = op.order_id)';
                break;
        }
    ...
    public function getShipping($data = array())
    {
		...
		SUM(ot.value) AS total, COUNT(o.order_id) AS `orders`
		FROM `'.DB_PREFIX.'product_to_beneficiary` cp
		LEFT JOIN	`'.DB_PREFIX.'order_product` op ON (cp.product_id = op.product_id)
		LEFT JOIN  `'.DB_PREFIX.'order` o  ON (o.order_id = op.order_id)
