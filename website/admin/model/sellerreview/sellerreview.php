...
class Modelsellerreviewsellerreview extends Model
{
    public function addsellerreview($data)
    {
        ...
        $this->db->query('INSERT INTO '.DB_PREFIX."sellerreview SET seller_name = '".$this->db->escape($data['seller_name'])."', seller_id = '".(int) $data['seller_id']."', text = '".$this->db->escape(strip_tags($data['text']))."', rating = '".(int) $data['rating']."', status = '".(int) $data['status']."', date_added = NOW()");
    ...
    public function editsellerreview($sellerreview_id, $data)
    {
        ...
        $this->db->query('UPDATE '.DB_PREFIX."sellerreview SET seller_name = '".$this->db->escape($data['seller_name'])."', seller_id = '".(int) $data['seller_id']."', text = '".$this->db->escape(strip_tags($data['text']))."', rating = '".(int) $data['rating']."', status = '".(int) $data['status']."', date_modified = NOW() WHERE sellerreview_id = '".(int) $sellerreview_id."'");
    ...
    {
        $sql = "SELECT r.sellerreview_id, CONCAT(pd.firstname, ' ', pd.lastname) AS title, r.seller_name, r.rating, r.status, r.date_added FROM ".DB_PREFIX.'sellerreview r LEFT JOIN '.DB_PREFIX.'customer pd ON (r.seller_id = pd.customer_id) LEFT JOIN '.DB_PREFIX.'customer_group_description cgd ON (pd.customer_group_id = cgd.customer_group_id) ';
        ...
        if (!empty($data['filter_seller_name'])) {
            $sql .= " AND r.seller_name LIKE '%".$this->db->escape($data['filter_seller_name'])."%'";
        }
