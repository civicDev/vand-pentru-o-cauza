...
class ModelsellerdashboardCustomer extends Model
{
    ...
    {
        $sql = 'SELECT COUNT(DISTINCT o.customer_id) AS total FROM `'.DB_PREFIX.'product_to_seller` cp
		LEFT JOIN  `'.DB_PREFIX.'order_product` op ON (cp.product_id = op.product_id) 
		LEFT JOIN  `'.DB_PREFIX."order` o ON (op.order_id = o.order_id) 
    ...
    public function getCustomersOnline($data = array())
    {
		...
		LEFT JOIN '.DB_PREFIX."seller_group_description cgd ON (c.seller_group_id = cgd.seller_group_id)
		WHERE co.customer_id != '0' AND cgd.seller_group_id 
    ...
    public function getTotalCustomersOnline($data = array())
    {
		...
		LEFT JOIN '.DB_PREFIX.'customer c ON (co.customer_id = c.customer_id)
		LEFT JOIN '.DB_PREFIX."seller_group_description cgd ON (c.seller_group_id = cgd.seller_group_id)
		WHERE co.customer_id != '0' AND cgd.seller_group_id != '0'
		";
